...
class UGATIT(object) :
    ...
    def build_model(self):
        ...
        self.SM_L1_loss_c1 = nn.SmoothL1Loss().to(self.device)
    ...
    def train(self):
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            fake_A2B, _, _, _ = self.genA2B(real_A)
            fake_B2A, _, _, _ = self.genB2A(real_B)
            ...
            fake_A2B, fake_A2B_cam_logit, _, fake_A2B_feature = self.genA2B(real_A)
            fake_B2A, fake_B2A_cam_logit, _, fake_B2A_feature = self.genB2A(real_B)

            fake_A2B2A, _, _, fake_A2B2A_feature = self.genB2A(fake_A2B)
            fake_B2A2B, _, _, fake_B2A2B_feature = self.genA2B(fake_B2A)

            fake_A2A, fake_A2A_cam_logit, _, _ = self.genB2A(real_A)
            fake_B2B, fake_B2B_cam_logit, _, _ = self.genA2B(real_B)
            ...
            G_content_loss_A = self.SM_L1_loss(fake_A2B_feature, fake_A2B2A_feature)
            G_content_loss_B = self.SM_L1_loss(fake_B2A_feature, fake_B2A2B_feature)
            ...
            if step % self.print_freq == 0:
                ...
                for _ in range(train_sample_num):
                    ...
                    fake_A2B, _, fake_A2B_heatmap, _ = self.genA2B(real_A)
                    fake_B2A, _, fake_B2A_heatmap, _ = self.genB2A(real_B)
