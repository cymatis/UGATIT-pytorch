...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_blocks=6, img_size=256, light=False):
        ...
        self.conv1x1_out = nn.Conv2d(ngf * mult * 2, ngf * mult, kernel_size=1, stride=1, bias=True)
        self.relu = nn.Mish()
        self.mish = nn.Mish()
    ...
    def forward(self, input):
        ...
        x = torch.cat([content_feature, g_conv],1)
        x = self.mish(self.conv1x1_out(x))
...
class Discriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=5):
        ...
        self.conv1x1_out = nn.Conv2d(ndf * mult * 2, ndf * mult, kernel_size=1, stride=1, bias=True)
        self.conv1x1_logit = nn.Conv2d(ndf * mult, 1, kernel_size=1, stride=1, bias=True)
        self.leaky_relu = nn.Mish()
        self.mish = nn.Mish()
