...
class UGATIT(object) :
    ...
    def train(self):
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            Discriminator_loss = D_loss_A + D_loss_B.to(torch.device("cuda:0"))
...
############
            ...
            Generator_loss = G_loss_A + G_loss_B.to(torch.device("cuda:0"))
            ...
            if step % self.print_freq == 0:
                ...
                for _ in range(train_sample_num):
                    ...
                    fake_B2A, _, fake_B2A_heatmap = self.genB2A(real_B) # 1
                    ...
                    fake_A2B2A, _, fake_A2B2A_heatmap = self.genB2A(fake_A2B_cuda) # 1
                    fake_B2A2B, _, fake_B2A2B_heatmap = self.genA2B(fake_B2A_cuda) # 0

                    fake_A2A, _, fake_A2A_heatmap = self.genB2A(real_A_cuda) # 1
                    fake_B2B, _, fake_B2B_heatmap = self.genA2B(real_B_cuda) # 0
                ...
                for _ in range(test_sample_num):
                    try:
                        real_A_raw, _ = testA_iter.next()
                    except:
                        testA_iter = iter(self.testA_loader)
                        real_A_raw, _ = testA_iter.next()

                    try:
                        real_B_raw, _ = testB_iter.next()
                    except:
    ...
    def test(self):
        ...
        for n, (real_A_raw, _) in enumerate(self.testA_loader):
        ...
        for n, (real_B_raw, _) in enumerate(self.testB_loader):
